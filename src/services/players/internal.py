...
class player(genericplayer):
    ...
    def __init__(self, id, playlistid, autoplay, aodevice, aooptions, bufsize,
        ...
        if bufferedao_present:
            self.audiodev = bufferedao.bufferedao(bufsize, self.SIZE, aodevice, rate=self.rate, byte_format=4, options=aooptions)
        ...
        try:
            ...
            if self.crossfading:
                ...
                if not manual and len(self.decodedsongs) == 2:
                    song1 = self.decodedsongs[0].song
                    song2 = self.decodedsongs[1].song
                    if song1.succeedsonalbum(song2):
